...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, unsigned char *vram, int xsize, int ysize)
{
	...
	for (i = 0; i < MAX_SHEETS; i++) {
		ctl->sheets0[i].flags = SHEET_UNUSE; /* 未使用マーク */
	...
	for (h = h0; h <= h1; h++) {
		...
		if ((sht->vx0 & 3) == 0) { /* 高速版 */
			...
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by;
				for (bx = bx0; bx < bx1 && (bx & 3); bx++) { /* 前の端数を1バイトずつ */
					vx = sht->vx0 + bx;
			...
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by; /* 画面上のy座標 */
				for (bx = bx0; bx < bx1; bx++) {
					vx = sht->vx0 + bx; /* 画面上のx座標 */
					if (map[vy * ctl->xsize + vx] == sid) {
						vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
					}
				}
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++) {
		...
		if (sht->col_inv == -1) { /* 透明色なし */
			if ((sht->vx0 & 3) == 0 && (bx0 & 3) == 0 && (bx1 & 3) == 0) { /* 高速版 */
				bx1 = (bx1 - bx0) / 4;
				sid4 = sid | sid << 8 | sid << 16 | sid << 24;
				for (by = by0; by < by1; by++) {
					vy = sht->vy0 + by;
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	sht->vx0 = vx0;
	sht->vy0 = vy0;
	if (sht->height >= 0) { /* もし表示中なら、新しい下じきの情報に沿って画面を描き直す */
...
{
	if (sht->height >= 0) {
		sheet_updown(sht, -1); /* 表示中なら非表示にする */
	}
	sht->flags = SHEET_UNUSE; /* 未使用マーク */
	return;
}
