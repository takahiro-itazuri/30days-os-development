	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			i = fifo32_get(&fifo);
			io_sti();
			if (key_win != 0 && key_win->flags == SHEET_UNUSE) { /* ウィンドウが閉じられた */
				if (shtctl->top == 1) { /* マウスと背景のみ */
					key_win = 0;
				}
				else {
			...
			if (256 <= i && i < 512) { /* キーボードデータ */
				...
				if (key_ctrl && s[0] == 'C' && key_win != 0) { /* Ctrl+C */
					task = key_win->task;
			...
			else if (768 <= i && i < 1024) { /* コンソール終了処理 */
